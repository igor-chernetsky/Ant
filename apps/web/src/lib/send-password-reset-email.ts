          ...
          <tr>
            <td style="padding:28px 28px 12px;">
              ...
              <p style="margin:0 0 20px;font-size:15px;line-height:1.6;color:#64748b;">
                We received a request to reset the password for your BuilTHAI account. Choose a new password with the button below.
              </p>
              <a href="${resetUrl}" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;font-weight:600;font-size:15px;padding:12px 22px;border-radius:10px;">
                Choose new password
              </a>
              <p style="margin:20px 0 0;font-size:13px;line-height:1.5;color:#64748b;">
                This link expires in 12 hours. If you did not request a reset, you can ignore this message.
              </p>
            </td>
          </tr>
          <tr>
...
export async function sendAppPasswordResetEmail(params: {
  userId: string;
  email: string;
}): Promise<{ ok: true } | { ok: false; message: string }> {
  const token = createPasswordResetToken(params.userId, params.email);
  if (!token) {
    return {
      ok: false,
      message:
        'Password reset is not configured (EMAIL_VERIFICATION_SECRET or SMTP).',
    };
  }
  ...
  try {
    await transport.sendMail({
      from: `"${fromName}" <${from}>`,
      to: params.email,
      subject: 'Reset your BuilTHAI password',
      html: buildPasswordResetEmailHtml(resetUrl),
      text: `Reset your BuilTHAI password:\n\n${resetUrl}\n\nThis link expires in 12 hours.`,
    });
    return { ok: true };
  } catch (error: unknown) {
    ...
    return {
      ok: false,
      message: 'Failed to send password reset email via SMTP.',
    };
  }
}
