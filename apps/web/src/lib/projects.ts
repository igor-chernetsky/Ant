...
export const CONSTRUCTION_PROJECT_TYPE_OPTIONS: Array<{
  ...
  {
    value: 'modernization_reconstruction',
    label: 'Modernization & reconstruction',
  },
...
export async function convertProjectToDesign(id: string): Promise<Project> {
  const response = await fetchWithAuth(
    `/api/projects/${encodeURIComponent(id)}/convert-to-design`,
    { method: 'POST' },
  );
  if (!response.ok) {
    const body = (await response.json().catch(() => null)) as {
    ...
    throw new Error(body?.message ?? 'Failed to convert project');
  }
  return response.json() as Promise<Project>;
}

export async function resumePendingProject(id: string): Promise<Project> {
  const response = await fetchWithAuth(
    `/api/projects/${encodeURIComponent(id)}/resume-pending`,
    { method: 'POST' },
  );
  if (!response.ok) {
    const body = (await response.json().catch(() => null)) as {
