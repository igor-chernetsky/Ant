...
import { PLATFORM_SOCIAL_LINKS } from './branding';
...
export class NotificationsService {
  ...
  private wrapEmail(
    ...
    const socialLinks = PLATFORM_SOCIAL_LINKS.map(
      (link) =>
        `<a href="${escapeHtml(link.href)}" style="color:#64748b;">${escapeHtml(link.label)}</a>`,
    ).join(' · ');
    footerParts.push(`Follow us: ${socialLinks}`);
    const footer =
...
<p style="margin:0;font-size:14px;line-height:1.55;color:#475569;white-space:pre-wrap;">${escapeHtml(trimmed)}</p>
  ...
  async notifyClientProgressClaimSubmitted(params: {
    ...
    await this.sendToUser({
      ...
      bodyHtml: `<p><strong>${escapeHtml(params.companyName)}</strong> submitted progress claim <strong>#${params.sequenceNumber}</strong> on <strong>${escapeHtml(params.projectTitle)}</strong>.</p><p>Payable this period: <strong>${escapeHtml(amountLabel)}</strong></p><p>Review the claim and approve or reject it on the project page.</p>`,
  ...
  async notifyContractorProgressClaimApproved(params: {
    ...
    await this.sendToUser({
      ...
      bodyHtml: `<p>The client approved progress claim <strong>#${params.sequenceNumber}</strong> on <strong>${escapeHtml(params.projectTitle)}</strong>.</p><p>Payable this period: <strong>${escapeHtml(amountLabel)}</strong></p>`,
      ctaHref: this.progressClaimsUrl(params.projectId),
